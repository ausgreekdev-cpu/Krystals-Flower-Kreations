...
npm test            # integration suite (41 tests), needs DATABASE_URL set
...
- **Local dev**: Postgres at `postgresql://krystal:krystal_dev_password@localhost:5432/krystals_flower_kreations?schema=public` (via `docker compose up -d db`, or the sandbox's embedded postgres under `/tmp/opencode/pg-embed/start.js`).
...
- Site is **not yet created** — user must import the repo and set env vars (see `.env.netlify.template` / `backend/.env.supabase`).
