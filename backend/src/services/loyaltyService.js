...
export async function loyaltyConfig() {
  const defs = { blossom: 100, garden: 500, earnRate: 1, redeemRate: 5, earnCapPerOrder: 1000 };
  try {
    const rows = await prisma.setting.findMany({
      where: { key: { in: ['loyalty_blossom_threshold', 'loyalty_garden_threshold', 'loyalty_earn_rate', 'loyalty_redeem_rate', 'loyalty_earn_cap'] } },
    });
