...
function Overview({data}){
  const orders = Array.isArray(data.orders) ? data.orders : []; const low = Array.isArray(data.low) ? data.low : []; const workshops = Array.isArray(data.workshops) ? data.workshops : [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="border rounded-xl p-4">
          ...
          <div className="mt-2 space-y-2">{(Array.isArray(orders)?orders:[]).slice(0,5).map(o=> <div key={o.id} className="flex justify-between text-xs border-b py-1"><span>{o.orderNumber} • {o.email}</span><span>${Number(o.total).toFixed(2)}</span></div>)}{orders.length===0 && <div className="text-xs text-gray-400">No orders yet</div>}</div>
        </div>
        <div className="border rounded-xl p-4">
          <h3 className="font-bold text-sm">Low stock alert</h3>
          <div className="mt-2 space-y-1">{(Array.isArray(low)?low:[]).slice(0,5).map(m=> <div key={m.id} className="text-xs flex justify-between"><span>{m.name}</span><span className="text-amber-600">{m.onHand}/{m.lowThreshold}</span></div>)}{low.length===0 && <div className="text-xs text-gray-400">All stocked</div>}</div>
        </div>
      </div>
    </div>
  );
...
function Orders({data}){ const orders=Array.isArray(data.orders)?data.orders:[]; return <div><h3 className="font-bold">Orders — {orders.length}</h3><div className="mt-3 max-h-[400px] overflow-auto space-y-2">{(Array.isArray(orders)?orders:[]).map(o=> <div key={o.id} className="flex justify-between text-xs border rounded-xl p-2"><span>{o.orderNumber} — {o.status} — {o.paymentStatus}</span><span>${Number(o.total).toFixed(2)}</span></div>)}{orders.length===0 && <div className="text-xs text-gray-400">No orders — check auth (maker+ token required)</div>}</div></div>; }
function Inventory({data}){ const levels=Array.isArray(data.levels)?data.levels:[]; const low=Array.isArray(data.low)?data.low:[]; const recon=Array.isArray(data.recon)?data.recon:[]; const drifted=recon.filter(r=>!r.ok); return <div><h3 className="font-bold">Inventory — {levels.length} levels, {low.length} low{drifted.length>0 && ` • ${drifted.length} drift`}</h3>{drifted.length>0 && <div className="mt-2 bg-amber-50 border border-amber-200 rounded-xl p-2 text-xs">⚠️ {drifted.length} variants drift: ledger vs onHand — check reconciliation below</div>}<div className="mt-3 max-h-[200px] overflow-auto space-y-1">{(Array.isArray(levels)?levels:[]).slice(0,20).map(l=> <div key={l.id} className="flex justify-between text-xs border rounded-lg p-2"><span>{l.product?.title || l.productId} {l.variant?`— ${l.variant.title}`:''} @ {l.location?.name}</span><span className={l.onHand<=5?'text-red-600':''}>{l.onHand}</span></div>)}{levels.length===0 && <div className="text-xs text-gray-400">No levels — seed inventory</div>}</div><div className="mt-4"><h4 className="font-bold text-xs">Reconciliation — ledger vs quantity</h4><div className="mt-2 max-h-[200px] overflow-auto space-y-1">{(Array.isArray(recon)?recon:[]).slice(0,10).map(r=> <div key={r.variantId} className={`flex justify-between text-xs border rounded-lg p-2 ${r.ok?'':'bg-red-50 border-red-200'}`}><span>{r.product} — {r.title}</span><span>{r.inventoryQuantity} vs ledger {r.ledgerTotal} {r.ok?'✓':`drift ${r.drift}`}</span></div>)}{recon.length===0 && <div className="text-xs text-gray-400">No variants</div>}</div></div></div>; }
function Products({data}){ const products=Array.isArray(data.products)?data.products:[]; return <div><h3 className="font-bold">Products — {products.length}</h3><div className="mt-3 grid sm:grid-cols-2 gap-3">{(Array.isArray(products)?products:[]).map(p=> <div key={p.id} className="border rounded-xl p-3"><div className="font-bold text-sm text-bloom-700">{p.title}</div><div className="text-xs text-gray-500">{p.sku} • ${Number(p.price).toFixed(2)} • {p.stockMode}</div><div className="text-xs text-gray-400">{p.variants?.length||0} variants • {p.images?.length||0} images</div></div>)}{products.length===0 && <div className="text-xs text-gray-400">No products</div>}</div></div>; }
...
function Workshops({data}){ const w=Array.isArray(data.workshops)?data.workshops:[]; return <div><h3 className="font-bold">Workshops — {w.length}</h3><div className="mt-3 space-y-2">{(Array.isArray(w)?w:[]).map(ws=> <div key={ws.id} className="border rounded-xl p-3"><div className="font-bold text-sm">{ws.title}</div><div className="text-xs text-gray-500">{ws.location} • {ws.capacity} cap • ${Number(ws.price).toFixed(2)}</div><div className="text-xs text-gray-400">{(ws.sessions||[]).length} sessions</div></div>)}{w.length===0 && <div className="text-xs text-gray-400">No workshops</div>}</div></div>; }
function Users({data}){ const users=Array.isArray(data.users)?data.users:[]; return <div><h3 className="font-bold">Users — {users.length}</h3><div className="text-xs text-gray-500">Requires developer role — if 403, login as admin@krystal.local / admin123 (seed)</div><div className="mt-3 space-y-1 max-h-[300px] overflow-auto">{(Array.isArray(users)?users:[]).map(u=> <div key={u.id} className="flex justify-between text-xs border rounded-lg p-2"><span>{u.name} • {u.email}</span><span className="bg-gray-100 rounded-full px-2 py-0.5">{u.role}</span></div>)}</div></div>; }
