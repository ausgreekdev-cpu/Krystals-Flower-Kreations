    ...
    <div className="max-w-6xl mx-auto px-4 py-8 grid md:grid-cols-2 gap-8">
      <div className="space-y-3">
        ...
        <div className="hidden md:flex gap-2 text-xs">
          <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`} target="_blank" className="border rounded-full px-3 py-1.5 hover:bg-surface3">Share FB</a>
          <a href={`https://www.instagram.com/`} target="_blank" className="border rounded-full px-3 py-1.5 hover:bg-surface3">IG Story</a>
          <button onClick={()=>{navigator.clipboard.writeText(window.location.href); setMsg('Link copied');}} className="border rounded-full px-3 py-1.5 hover:bg-surface3">Copy link</button>
        </div>
      </div>
      <div>
