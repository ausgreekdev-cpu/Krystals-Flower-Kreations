...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <form onSubmit={submit} className="w-full max-w-sm bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-4">
        <div className="flex items-center gap-2">
          <span className="w-9 h-9 rounded-xl bg-royal-600 flex items-center justify-center text-white font-black">K</span>
          <div>
            <h1 className="font-black text-gray-900 leading-tight">Studio sign in</h1>
            <p className="text-[11px] text-gray-500">Krystal's Flower Kreations</p>
          </div>
        </div>
        ...
        <label className="block text-sm">
          ...
          <input type="email" required autoComplete="username" autoFocus value={email} onChange={(e) => setEmail(e.target.value)}
            className="mt-1 w-full border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-royal-600" />
        </label>
        <label className="block text-sm">
        ...
        {import.meta.env.DEV && <p className="text-[11px] text-gray-400 text-center">Dev seed: admin@krystal.local / admin123</p>}
        <a href="/" className="block text-center text-xs text-gray-500 hover:underline">← Back to shop</a>
      </form>
    </div>
  );
}
