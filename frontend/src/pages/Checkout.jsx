...
export default function Checkout(){
  const [form,setForm]=useState({ email:'', shippingName:'', shippingAddress:'', shippingSuburb:'', shippingState:'WA', shippingPostcode:'', discountCode:'', customerNote:'', paymentMethod:'pickup', acceptTerms:false });
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <form className="mt-6 grid gap-4 bg-surface2 p-6 rounded-2xl border" onSubmit={submit}>
        ...
        <select value={form.paymentMethod} onChange={e=>setForm({...form,paymentMethod:e.target.value})} className="border rounded-xl px-3 py-2 bg-surface2">{methods.map(m=> <option key={m.value} value={m.value}>{m.label}</option>)}</select>
        ...
        <p className="text-xs text-muted">ABN on invoice. Stripe disabled — cash/bank/pickup only for now. To re-enable: set STRIPE_ENABLED=true + STRIPE_SECRET_KEY.</p>
      </form>
    </div>
  );
}
