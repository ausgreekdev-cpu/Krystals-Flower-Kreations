...
export default function Loyalty(){
  ...
  async function load(){
    ...
    // leaderboard public
    fetch(`${(import.meta.env.VITE_API_URL||'')}/api/loyalty/leaderboard`).then(r=>r.json()).then(d=> setBoard(Array.isArray(d)?d: (Array.isArray(d.board)?d.board:[]))).catch(()=> setBoard([]));
    if(!token) return;
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white border rounded-2xl p-6">
        {!me ? <div className="text-sm text-gray-600">Login to see your points. <a href="/login" className="underline text-bloom-500">Staff login</a> or continue as guest for leaderboard.</div> : (
          <div>
            ...
            <div className="mt-4 space-y-2 max-h-40 overflow-auto">
              {(Array.isArray(me.transactions)?me.transactions:[]).map(t=> <div key={t.id} className="flex justify-between text-xs border-b py-1"><span>{t.reason}</span><span className={t.pointsDelta>0?'text-green-600':'text-red-600'}>{t.pointsDelta>0?'+':''}{t.pointsDelta}</span></div>)}
              {(!me.transactions||(Array.isArray(me.transactions) && me.transactions.length===0)) && <div className="text-xs text-gray-400">No transactions yet — earn with first order.</div>}
            </div>
          </div>
        )}
      </div>
      <div className="bg-bloom-700 text-white rounded-2xl p-6">
        ...
        <div className="mt-3 space-y-1">
          {(Array.isArray(board)?board:[]).map((b,i)=> <div key={i} className="flex justify-between text-sm bg-white/10 rounded-xl px-3 py-2"><span>#{i+1} {b.email}</span><span>{b.points} pts • {b.tier}</span></div>)}
          {(!Array.isArray(board) || board.length===0) && <div className="text-xs opacity-70">No points yet — be first.</div>}
        </div>
      </div>
      <div className="bg-white border rounded-2xl p-6">
        ...
        <ul className="text-xs text-gray-600 list-disc pl-4 mt-2 space-y-1">
          ...
          <li>Review photo: staff award 10 pts when your review is approved</li>
