  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div><h1 className="text-2xl md:text-3xl font-black text-ink">Shop — paper blooms & armature art</h1><p className="text-sm text-muted mt-1">Perth studio • Made-to-order 3-7 days • Free Perth delivery over $150</p></div>
      ...
      <div className="mt-6 flex justify-between items-center text-sm">
        <span className="text-muted">{filtered.length} blooms</span>
        <span className="text-muted hidden md:inline">Optimised for desktop — hover for zoom • Configurator for custom</span>
      </div>
      {filtered.length===0 && !loading && !error && <p className="text-center text-muted mt-8 py-12 bg-surface2 border rounded-2xl">No blooms yet — seed the backend and refresh.</p>}
    </div>
  );
}
