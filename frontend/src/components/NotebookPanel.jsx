...
export default function NotebookPanel({ compact=false, query='' }){
  ...
  if(compact){
    return (
      <div className="bg-surface2 border rounded-xl p-3 flex justify-between items-center">
        <span className="text-xs font-bold text-ink">📓 Notebook — {query || 'Perth studio research'}</span>
        <a href={href} target="_blank" rel="noopener" className="text-xs bg-bloom-500 text-white px-3 py-1 rounded-full">Open →</a>
      </div>
    );
  }
  return (
    <details className="bg-surface2 border rounded-2xl">
      <summary className="px-4 py-3 font-bold text-sm cursor-pointer">📓 NotebookLM — {query || 'Cricut settings & BOM'}</summary>
      <div className="p-4 border-t">
        <p className="text-xs text-muted">Ask in NotebookLM: “What pressure for {query || '65lb Blush'} in Perth humidity?” — sources include paper stocks, armature guides.</p>
